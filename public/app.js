...
async function grantAllPermissions() {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      function(pos) {
        var ls = document.getElementById('perm-location-status');
        if (ls) { ls.textContent = '✅ مسموح'; ls.style.background = 'rgba(0,230,118,0.15)'; ls.style.color = 'var(--success)'; }
        userLocation = { lat: pos.coords.latitude, lng: pos.coords.longitude, accuracy: pos.coords.accuracy };
      },
      function() {
        var ls = document.getElementById('perm-location-status');
        if (ls) { ls.textContent = '❌ مرفوض'; ls.style.background = 'rgba(255,61,113,0.15)'; ls.style.color = 'var(--danger)'; }
      },
...
function dismissPermissions() {
  ...
  updatePushBtnUI(('Notification' in window) ? Notification.permission : 'denied');
}
